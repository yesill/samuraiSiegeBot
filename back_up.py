...
def fCheckBandage(liste):   #liste[0] = r , liste[1] = g , liste[2] = b
    ...
    bandage = False
    tb = ((225, 225, 225), (235, 193, 0), (210, 100, 0))    #taban
    tv = ((240, 240, 240), (245, 203, 5), (235, 130, 15))   #tavan
...
def fAttack():
    ...
    for i in range(0,2):
        ...
        #troop deploy
        delay = 0.2
        def fAttackClick(x,y):
            time.sleep(delay)
            fClick(x,y)
        fAttackClick(644,541)
        fAttackClick(284,396)
        fAttackClick(1166,262)
        fAttackClick(1187,750)
        fAttackClick(1022,674)
        fAttackClick(135,689)
        fAttackClick(20,265)
    ...
    def fHomeTownControl():
        ...
        def fRetreat():
            while True:
                ...
                if (((L0 >= yesil[0]) and (L0 <= yesil[1])) and ((L1 >= kirliBeyaz[0]) and (L1 <= kirliBeyaz[1])) and ((L2 >= yesil[0]) and (L2 <= yesil[1]))):
                    fClick(746,866) #left button = yes
                    fBackHome()
                    break
                else:
                    break
        ...
        while True:
            ...
            if ((tempList >= battleButtonColorBorders[0]) and (tempList <= battleButtonColorBorders[1])):
                os.system('cls')
                print("Battle..!")
                fClick(1454,996)    #battle button
                break
            else:
                if (fGetSS(770,186) != (167,23,23)):
                    fClick(770,186)     #return the village or retreat clicked
...
def fPlayTime():
    while True:
        ...
        L2 = fGetSS(786,866)
        if (((L0 >= yesil[0]) and (L0 <= yesil[1])) and ((L1 >= kirliBeyaz[0]) and (L1 <= kirliBeyaz[1])) and ((L2 >= yesil[0]) and (L2 <= yesil[1]))):
            fClick(786,866) #right button = yes
            break
        else:
            break
...
def fBackHome():
    while True:     #home town donme
        ...
        if ((tempList >= yesil[0]) and (tempList <= yesil[1])):   #continue butonun oldugu yer yesil ise
            fClick(640,880) #continue button
            break
